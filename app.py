...
def sniper_ele():
    ...
    if not lista_p: return jsonify({"error": "Nenhum produto configurado."})
    ...
    for idx, prod in enumerate(lista_p):
        tar = cfg[seg][t_ee]['tar'][prod['tipo']]
        nos_en_t = sum(safe_float(request.form.get(f'cons_p{i}')) * (prod[f'p{i}'] if t_ee == 'EV' else prod[f'p{i}'] + tar[f'p{i}']) for i in range(1, 5))
        nos_total = nos_en_t + nos_p_t
        resultados.append({"idx": idx, "nome": prod['nome'], "nos_total": round(nos_total, 2), "poup_anual": round(((cli_total - nos_total) / max(dias, 1)) * 365, 2)})

    resultados.sort(key=lambda x: x['poup_anual'], reverse=True)
    return jsonify({ "cli_total": round(cli_total, 2), "top3": resultados[:3] })
...
# ==========================================
# NOTAS E CRM (ATUALIZADO COM AGENDA)
# ==========================================
...
def save_note():
    ...
    if note_id:
        for n in user_notes:
            if n['id'] == note_id:
                n.update({
                    'title': req.get('title', ''), 'subtitle': req.get('subtitle', ''), 
...
def delete_note(note_id):
    token = request.args.get('token') or session.get("admin_logged_in")
    data = load_notes()
    data[str(token)] = [n for n in data.get(str(token), []) if n['id'] != str(note_id)]
    save_notes(data)
...
def admin_notas():
    ...
    return render_template('admin_notas.html', all_notes=load_notes())
...
def export_leads():
    ...
    writer.writerow(['Operador', 'Estado', 'Titulo', 'Subtitulo', 'NIPC', 'Telefone', 'Data Ligar', 'Hora Ligar', 'Descricao'])
    for token, notes in load_notes().items():
        for n in notes: writer.writerow([token, n.get('status', '').upper(), n.get('title', ''), n.get('subtitle', ''), n.get('nipc', ''), n.get('phone', ''), n.get('followup_date', ''), n.get('followup_time', ''), n.get('desc', '').replace('\n', ' | ')])
...
# ==========================================
# MOTOR DO CHAT E BROADCAST (SQL READY)
# ==========================================
...
# ROTA PARA ENVIAR O MEGAFONE (ADMIN)
...
def get_chat_status():
    ...
    # Broadcast Global para avisar todos os ecrãs
